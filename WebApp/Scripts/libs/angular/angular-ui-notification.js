...
angular.module('ui-notification').provider('Notification', function () {
	...
	this.$get = ["$timeout", "$http", "$compile", "$templateCache", "$rootScope", "$injector", "$sce", "$q", "$window", function ($timeout, $http, $compile, $templateCache, $rootScope, $injector, $sce, $q, $window) {
		...
		var notify = function (args, t) {
			...
			function processNotificationTemplate(template) {
				...
				var reposite = function () {
					var j = 0;
					var k = 0;
					startTop = args.startTop != undefined ? args.startTop : options.startTop;
					startRight = args.startRight != undefined ? args.startRight : options.startRight;
		...
		notify.sessionExpired = function(args) {
			return this(args, 'session-expired');
		};
		notify.loginSuccess = function (args) {
			return this(args, 'login-success');
		};
